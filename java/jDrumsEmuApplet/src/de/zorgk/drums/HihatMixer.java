...
public class HihatMixer implements HitInterface {
	...
	public HihatMixer(HitInterface oord, HitInterface  otop,HitInterface  cord,HitInterface  ctop, HitInterface bel) {
		this.bel = bel;
		this.cord = cord;
		this.oord = oord;
		this.ctop = ctop;
	...
	public void hit(long frame) {
		...
		cord.hit(frame,dB+functionTables.GaintodB(rimXbel*ordXtop*(1.f-openXclosed)));
		oord.hit(frame,dB+functionTables.GaintodB(rimXbel*ordXtop*openXclosed));
		ctop.hit(frame,dB+functionTables.GaintodB(rimXbel*(1.f-ordXtop)*(1.f-openXclosed)));
		otop.hit(frame,dB+functionTables.GaintodB(rimXbel*(1.f-ordXtop)*openXclosed));
	}
	...
	public long getParameterNbrName(String name) {
		if (name == "openXclosed") return 0;
		if (name == "ordXtop") return 1;
