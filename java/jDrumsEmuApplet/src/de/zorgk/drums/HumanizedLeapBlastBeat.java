...
public class HumanizedLeapBlastBeat implements RiffInterface, Cloneable {
	...
	public HumanizedLeapBlastBeat(long bpm, long hits, float accuracyInMs, float openClosed, float openClosed2Sigma, float ordTop, float ordTop2Sigma, SamplerSetup sampler) {
	...
	public boolean timeElapse(long framestart, long framenextstart,
		...
		if (!started) {
			...
			this.hihat.hit(framestart+randomGenerator.nextInt(uniformAccuracy));
			this.nextKick = 1;
